...
export function create() {
    ...
    graphics.generateTexture('whiteRect', 50, 10);
    graphics.destroy();
    ...
    player = this.physics.add.sprite(225, 680, 'whiteRect')
...
export function update() {
    ...
    if (gameStarted) {
        if (lastPlayerY >= player.y) {
            lastPlayerY = player.y
        } else if (player.y - lastPlayerY >= 500 || ground.y - player.y <= 20) {
